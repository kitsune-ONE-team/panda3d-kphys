...
chain_create(void)
{
    ...
    if (chain == NULL)
    {
        IKAPI.log.message("eFailed to allocate chain: out of memory");
        return NULL;
    }
    ...
    VECTOR_FOR_EACH(effector_nodes_list, struct ik_node_t*, p_effector_node)
        ...
        for (; node != NULL; node = node->parent)
        {
            ...
            if (current_marking == NULL)
            {
                ...
                {
                    IKAPI.log.message("eRan out of memory while marking involved nodes");
        ...
        case MARK_SECTION:
            ...
            if ((marked_children_count == 2 || node_current->effector != NULL) && node_current != node_base)
            {
                ...
                if (chain_current == NULL) /* First chain in the tree? */
                {
                    ...
                    if (child_chain == NULL)
                    {
                        IKAPI.log.message("eFailed to create base chain: Ran out of memory");
                        return IK_RAN_OUT_OF_MEMORY;
                    }
                ...
                else /* This is not the first chain in the tree */
                {
                    ...
                    if (child_chain == NULL)
                    {
                        IKAPI.log.message("eFailed to create child chain: Ran out of memory");
                        return IK_RAN_OUT_OF_MEMORY;
                    }
                    chain_construct(child_chain);
                }
                ...
                for (node = node_current; node != node_base; node = node->parent)
                    ...
                    {
                        IKAPI.log.message("eFailed to insert node into chain: Ran out of memory");
                        return IK_RAN_OUT_OF_MEMORY;
                    }
                   ...
                   const struct vector_t* effector_nodes_list)
{
    ...
    IKAPI.log.message("dThere are %d effector(s) involving %d node(s). %d chain(s) were created",
