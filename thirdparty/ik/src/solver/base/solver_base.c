...
ik_solver_base_rebuild(struct ik_solver_t* solver)
{
    ...
    if (solver->tree == NULL)
    {
        IKAPI.log.message("eNo tree to work with. Did you forget to set the tree with ik_solver_set_tree()?");
     ...
     */
    IKAPI.log.message("dRebuilding effector nodes list");
    vector_clear(&solver->effector_nodes_list);
    if ((result = recursively_get_all_effector_nodes(
            solver->tree,
            &solver->effector_nodes_list)) != IK_OK)
    {
        IKAPI.log.message("eRan out of memory while building the effector nodes list");
        return result;
    }
                                 ...
                                 ik_solver_iterate_node_cb_func callback)
{
    if (solver->tree == NULL)
    {
        IKAPI.log.message("wWarning: Tried iterating the tree, but no tree was set");
        return;
    }
